...
func (s *supervisor) applyInner(ctx context.Context, eventHandler func(Event), declaredResources *declared.Resources) (ObjectStatusMap, *stats.SyncStats) {
	...
	options := apply.ApplierOptions{
		...
		// SkipUpdateAnnotations defines which annotations on an object should indicate
		// that an object should not be updated. i.e. "ignore mutation"
		SkipUpdateAnnotations: map[string]string{
			metadata.LifecycleMutationAnnotation: metadata.IgnoreMutation,
		},
	}
