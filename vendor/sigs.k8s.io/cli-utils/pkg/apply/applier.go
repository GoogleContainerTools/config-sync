	...
	go func() {
		...
		applyFilters := []filter.ValidationFilter{
			...
			filter.PreventUpdateFilter{
				SkipUpdateAnnotations: options.SkipUpdateAnnotations,
				Client:                a.metadataClient,
				Mapper:                a.mapper,
			},
...
type ApplierOptions struct {
	...
	// SkipUpdateAnnotations is a set of configurable annotations. If any annotation
	// key/value pair is present on the object, it will be created but not updated
...
func setDefaults(o *ApplierOptions) {
	...
	if o.SkipUpdateAnnotations == nil {
		o.SkipUpdateAnnotations = make(map[string]string)
	}
}
